...
public class DatosPersona_Pelicula_Serie_Productor {
    ...
    public static List<PPS_Productor> datosListar() throws Exception {
        ...
        try {
            l = new ArrayList<>();
            c.conectar();
            String sql = "Select * from persona_pelicula_serie_productor;";
            PreparedStatement st = c.getCn().prepareStatement(sql);
            rs = st.executeQuery();
            while (rs.next()) {
                PPS_Productor entity = new PPS_Productor(rs.getInt("ppsp_id"), new Persona(rs.getInt("per_id")), new Pelicula_Serie(rs.getInt("ps_id")),rs.getString("ppsp_tipo"));
                l.add(entity);
            }
        } catch (Exception ex) {
