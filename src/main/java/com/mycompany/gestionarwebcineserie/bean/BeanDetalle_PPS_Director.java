...
@ManagedBean
@ViewScoped
public class BeanDetalle_PPS_Director {

    /**
     * Variable y objetos de clases determinadas.
     */
    private String[] selectTipo = {"Director Principal","Director Fotografia","Director de Libretos","Director de Compositor","Director de Montaje"};
    private Persona entityPer = new Persona();
    private Pelicula_Serie entityPS = new Pelicula_Serie();
    private PPS_Director entity = new PPS_Director();
    private List<PPS_Director> listaEntities;
    ...
    public PPS_Director getEntity() {
        return entity;
    }

    public void setEntity(PPS_Director entity) {
        this.entity = entity;
    }

    public List<PPS_Director> getListaEntities() {
        return listaEntities;
    }

    public void setListaEntities(List<PPS_Director> listaEntities) {
    ...
    private void filtrarElementosRepetidos() {
        List<PPS_Director> newList = this.listaEntities;
        List<PPS_Director> oldList = this.listaEntities;
        List<PPS_Director> temp = new ArrayList<>();
        if (newList.size() > 0) {
    ...
    public void listar(boolean valor) throws Exception {
        try {
            if (!valor) {
                if (!isPostBack()) {
                    listaEntities = Control_PPS_Director.control_listar();
                }
            } else {
                listaEntities = Control_PPS_Director.control_listar();
            }
     ...
     */
    public void quitarElemento(PPS_Director objEntity) throws Exception {
        try {
            Control_PPS_Director.control_eliminar(objEntity);
            ...
            if (this.listaEntities.size() > 0) {
                {
                    ...
                    Control_PPS_Director.control_RegistrarLista(listaEntities);
        ...
        try {
            if (this.entityPS != null) {
                if (this.entityPer != null) {
                    PPS_Director objEntity = this.entity;
                    objEntity.setPelicula_serie(this.entityPS);
                    objEntity.setPersona(this.entityPer);
                    boolean rest = Control_PPS_Director.control_GetExitenciaTupla(objEntity);
                    if (rest) {
        ...
        try {
            ...
            if (this.entityPS != null) {
                if (this.entityPer != null) {
                    ...
                    this.listaEntities.clear();
                    this.listaEntities = Control_PPS_Director.control_GetAllTuplas(objEntity);
                } else {
     ...
     * @throws Exception
     */
    public void leerID(PPS_Director objEntity) throws Exception {
        try {
            PPS_Director temp = Control_PPS_Director.control_leerID(objEntity);
            if (temp != null) {
                this.entity = temp;
