...
import com.mycompany.gestionarwebcineserie.control.Control_PPS_Screenwriter;
...
@ManagedBean
@ViewScoped
public class BeanDetalle_PPS_Screenwriter {
    ...
    private PPS_Screenwriter entity = new PPS_Screenwriter();
    private List<PPS_Screenwriter> listaEntities;
    ...
    public PPS_Screenwriter getEntity() {
        return entity;
    }

    public void setEntity(PPS_Screenwriter entity) {
        this.entity = entity;
    }

    public List<PPS_Screenwriter> getListaEntities() {
        return listaEntities;
    }

    public void setListaEntities(List<PPS_Screenwriter> listaEntities) {
    ...
    private void filtrarElementosRepetidos() {
        List<PPS_Screenwriter> newList = this.listaEntities;
        List<PPS_Screenwriter> oldList = this.listaEntities;
        List<PPS_Screenwriter> temp = new ArrayList<>();
        if (newList.size() > 0) {
    ...
    public void listar(boolean valor) throws Exception {
        try {
            if (!valor) {
                if (!isPostBack()) {
                    listaEntities = Control_PPS_Screenwriter.control_listar();
                }
            } else {
                listaEntities = Control_PPS_Screenwriter.control_listar();
            }
     ...
     */
    public void quitarElemento(PPS_Screenwriter objEntity) throws Exception {
        try {
            Control_PPS_Screenwriter.control_eliminar(objEntity);
            ...
            if (this.listaEntities.size() > 0) {
                {
                    ...
                    Control_PPS_Screenwriter.control_RegistrarLista(listaEntities);
        ...
        try {
            if (this.entityPS != null) {
                if (this.entityPer != null) {
                    PPS_Screenwriter objEntity = this.entity;
                    objEntity.setPelicula_serie(this.entityPS);
                    objEntity.setPersona(this.entityPer);
                    boolean rest = Control_PPS_Screenwriter.control_GetExitenciaTupla(objEntity);
                    if (rest) {
        ...
        try {
            ...
            if (this.entityPS != null) {
                if (this.entityPer != null) {
                    ...
                    this.listaEntities.clear();
                    this.listaEntities = Control_PPS_Screenwriter.control_GetAllTuplas(objEntity);
                } else {
     ...
     * @throws Exception
     */
    public void leerID(PPS_Screenwriter objEntity) throws Exception {
        try {
            PPS_Screenwriter temp = Control_PPS_Screenwriter.control_leerID(objEntity);
            if (temp != null) {
                this.entity = temp;
